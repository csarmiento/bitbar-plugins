...
def main():
    try:
        ...
        print(get_color_by_min(f'{download_str.strip()} :arrow_down:', download_val, 50))
        print(get_color_by_min(f'{upload_str.strip()} :arrow_up:', upload_val, 7))
        print(get_color_by_max(f'{latency_str.strip()} :zap:', latency_val, 50))
        print(get_color_by_max(f'{packet_loss_str.strip()} :no_mobile_phones:', packet_loss_val, 1))
...
def get_color_by_max(status_str: str, current_val: float, max_value: float) -> str:
    return status_str + '|color=green' if current_val <= max_value else status_str + '|color=red'
